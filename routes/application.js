...
const { verifyStaffTokenAndAuth, verifyStaffTokenAndAdmin, verifyStudentTokenAndAuth, verifyStudentToken, verifyStaffToken, verifyStudentRole } = require("./verifyToken");
...
//update
router.put("/:id", verifyStaffTokenAndAuth || verifyStudentTokenAndAuth, async (req, res) => {
    try {
...
router.get("/app/active", verifyStaffTokenAndAdmin, async (req, res) => {
    const order = req.query.order;
    try {
        let applications;
        if(order === "asce") {
            applications = await Application.find({isActive: true}).sort({"createdAt":1});
        } else if (order === "desc") {
            applications = await Application.find({isActive: true}).sort({"createdAt":-1});
        } else {
            applications = await Application.find({isActive: true});
        }
        res.status(200).json(applications);
    } catch (error) {
        console.log(error);
